       ...
       <div class="overflow-x-auto">
         <table class="w-full text-sm text-left">
           <thead class="text-xs text-gray-400 uppercase bg-white/5">
             <tr>
               <th scope="col" class="px-6 py-4 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">统计类型</th>
           ...
           <tbody>
             <tr v-for="item in paginatedData" :key="item.name" class="border-b border-white/10 hover:bg-white/5 transition-colors duration-200">
               <td class="px-6 py-4">
               ...
               <td class="px-6 py-4">
                 <span class="px-3 py-1.5 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 text-blue-300 border border-blue-400/30 rounded-full text-sm font-bold backdrop-blur-sm">
                   <i class="fas fa-cube mr-1 text-blue-400"></i>
               ...
               <td class="px-6 py-4">
                 <span class="px-3 py-1.5 bg-gradient-to-r from-purple-500/20 to-indigo-500/20 text-purple-300 border border-purple-400/30 rounded-full text-sm font-medium backdrop-blur-sm">
...
import { getEquipmentOverview } from '@/api/equipmentStatistics'
...
const statsData = ref({
  ...
  total_applications_growth: 0,
  approved_applications_growth: 0,
  pending_applications_growth: 0,
  rejected_applications_growth: 0
})
...
// 获取图表配置
 const getChartOption = () => {
   ...
   // 渐变色配置 - 参考CategoryEquipmentStats.vue
   const gradients = [
     ['#3B82F6', '#1D4ED8'], // 蓝色渐变
     ['#10B981', '#047857'], // 绿色渐变
     ['#F59E0B', '#D97706'], // 黄色渐变
   ...
   return {
     ...
     legend: {
       ...
       padding: [0, 20],
       itemGap: 12,
       // 图例显示过多时的处理 - 参考CategoryEquipmentStats.vue
     ...
     series: [
       {
         ...
         minAngle: 5,
         // 饼图扇区数据 - 参考CategoryEquipmentStats.vue
         ...
         // 饼图标签设置 - 参考CategoryEquipmentStats.vue
         label: {
           show: true,
           position: 'outside',
           color: '#e2e8f0',
         ...
         // 连接线样式 - 参考CategoryEquipmentStats.vue
         labelLine: {
           show: true,
           length: 25,
           length2: 40,
...
const loadData = async () => {
  ...
  try {
    ...
    // 调用API
    const response = await getEquipmentOverview(params)
    ...
    if (response.success && response.data) {
      ...
      // 转换数据格式用于图表展示 - 使用部门数据
      chartData.value = departmentData.map(item => ({
        name: item.name || '未知部门',
        total_quantity: item.total_quantity || 0,
        application_count: item.application_count || 0,
        percentage: item.percentage || 0,
        stat_type: item.stat_type || '部门'
      }))
      
      // 表格数据直接使用部门数据
      tableData.value = departmentData
      total.value = departmentData.length
      
      // 计算统计数据（基于部门数据）
      const totalQuantity = departmentData.reduce((sum, item) => sum + (item.total_quantity || 0), 0)
      const totalApplications = departmentData.reduce((sum, item) => sum + (item.application_count || 0), 0)
      
      // 更新统计卡片数据
      Object.assign(statsData.value, {
        total_applications: totalQuantity,
        approved_applications: totalApplications,
        pending_applications: 0,
        rejected_applications: 0,
        total_applications_growth: 0,
        approved_applications_growth: 0,
    ...
    console.error('加载设备概览数据失败:', error)
    chartData.value = []
    tableData.value = []
    total.value = 0
    // 重置统计数据
...
const exportData = async () => {
  ...
  try {
    ...
    // 使用当前表格数据构建CSV（适配部门数据格式）
    const headers = ['统计类型', '部门名称', '总数量', '申请次数', '占比']
    const csvContent = [
      headers.join(','),
    ...
    link.setAttribute('download', `设备分类统计_${new Date().toLocaleDateString('zh-CN')}.csv`)
